 ...
 */
export async function findAvailablePort(startPort = 9000) {
 ...
 */
export async function getAvailablePorts(count, startPort = 9000) {
  const ports = [];
  let next = startPort;
  for (let i = 0; i < count; i++) {
    const port = await findAvailablePort(next);
    ports.push(port);
    next = port + 1;
  }
  return ports;
